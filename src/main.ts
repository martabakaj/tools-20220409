...
async function getApiResponse(url: string): Promise<any> {
    ...
    const json: Object  = await response.json(); // lub     const json: any  = await response.json(); /* any dodajemy ka nie wiemy jakiego typy zmienne bedziemy mieć*/
    return json;
  }
